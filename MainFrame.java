...
public class MainFrame extends JFrame {
    private JTable tabelDashboard, tabelStok, tabelHistori;
    ...
    private TableRowSorter<DefaultTableModel> sorterStok, sorterHistori; 

    // --- PALET WARNA TAILWIND CSS (SaaS LOOK - ORIGINAL) ---
    ...
    private void styleModernTable(JTable table) {
        table.setFont(fontUtama); table.setRowHeight(40); table.setShowGrid(false); table.setIntercellSpacing(new Dimension(0, 0));
    ...
    // =========================================
    // PANEL ANALITIK & HISTORI (TATA LETAK SCROLL BAWAH)
    // =========================================
    private JPanel createAnalitikPanel() {
        JPanel content = new JPanel(new BorderLayout(0, 20)); content.setOpaque(false);
        
        // 1. KARTU SUMMARY (TETAP DI ATAS SUPAYA SELALU KELIHATAN)
        pnlSummaryCards = new JPanel(new GridLayout(1, 4, 20, 0)); pnlSummaryCards.setOpaque(false); 
        content.add(pnlSummaryCards, BorderLayout.NORTH);

        // 2. BAGIAN TENGAH (Scrollable)
        JPanel pnlTengah = new JPanel(); 
        pnlTengah.setLayout(new BoxLayout(pnlTengah, BoxLayout.Y_AXIS)); 
        pnlTengah.setOpaque(false); 
        ...
        tabelHistori = new JTable(); styleModernTable(tabelHistori);
        refreshHistoriTabel(); 

        txtSearchHistori.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            public void insertUpdate(javax.swing.event.DocumentEvent e) { search(); }
            public void removeUpdate(javax.swing.event.DocumentEvent e) { search(); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { search(); }
            private void search() {
                String text = txtSearchHistori.getText();
                if (text.trim().length() == 0) { sorterHistori.setRowFilter(null); } 
                else { sorterHistori.setRowFilter(RowFilter.regexFilter("(?i)" + text)); }
            }
        });

        JScrollPane scrollHistori = new JScrollPane(tabelHistori); scrollHistori.setBorder(BorderFactory.createLineBorder(borderColor));
        scrollHistori.getViewport().setBackground(bgCard);
        
        JPanel pnlAksiHistori = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0)); pnlAksiHistori.setOpaque(false);
        JButton btnHapusHistori = styleActionBtn("Hapus Transaksi", new Color(239, 68, 68), new Color(220, 38, 38)); 
        
        btnHapusHistori.addActionListener(e -> {
            int row = tabelHistori.getSelectedRow();
            if (row == -1) { JOptionPane.showMessageDialog(this, "Pilih transaksi di tabel yang ingin dihapus!"); return; }
            int modelRow = tabelHistori.convertRowIndexToModel(row);
            
            if (JOptionPane.showConfirmDialog(this, "Yakin ingin menghapus riwayat transaksi ini?\nBudgetmu akan otomatis kembali/menyesuaikan.", "Konfirmasi", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                try { 
                    int idTransaksi = Integer.parseInt(tabelHistori.getModel().getValueAt(modelRow, 0).toString());
        ...
        pnlAksiHistori.add(btnHapusHistori);
        pnlHistoriArea.add(pnlSearchHistori, BorderLayout.NORTH);
        pnlHistoriArea.add(scrollHistori, BorderLayout.CENTER);
        pnlHistoriArea.add(pnlAksiHistori, BorderLayout.SOUTH);
        ...
        JScrollPane scrollUtama = new JScrollPane(pnlTengah);
        ...
        try {
            ...
            if (dataKat.isEmpty()) lblInsightSmarter.setText("<html>Belum ada transaksi. Keuangan stabil!</html>");
                ...
                for (java.util.Map.Entry<String, Double> e : data5Hari.entrySet()) {
                    JPanel barPnl = new JPanel(new BorderLayout(15, 0)); barPnl.setOpaque(false); barPnl.setBorder(new EmptyBorder(5, 0, 5, 0));
                    ...
                    bar.setString(String.format("Rp %,.0f", e.getValue()).replace(",", ".")); 
    ...
    private JPanel createSettingsPanel() {
        ...
        gbc.gridwidth = 1; addFormLabel(content, "Batas Per Kategori", gbc, 0, 5); txtSetKategori = createWebTextField(); gbc.gridx = 1; gbc.gridy = 5; content.add(txtSetKategori, gbc);
        ...
        btnSaveGeneral.addActionListener(e -> {
            ...
            } catch (Exception ex) { JOptionPane.showMessageDialog(this, "Input harus angka atau titik!"); } 
        });

        // Logika Alarm Validasi Budget Kategori
        btnKat.addActionListener(e -> { 
            try { 
                String katBersih = txtSetKategori.getText().replace(".", "").replace(",", "");
                double inputBatasBaru = Double.parseDouble(katBersih);
                int idKatTerpilih = Integer.parseInt(listSemuaKategori.get(comboKategoriSet.getSelectedIndex())[0]);
                String namaKatTerpilih = listSemuaKategori.get(comboKategoriSet.getSelectedIndex())[1];

                double budgetGlobal = DatabaseHelper.getBudgetGlobal();
                double totalAlokasiLain = DatabaseHelper.hitungSisaBudgetUntukKategori(idKatTerpilih);

                if ((totalAlokasiLain + inputBatasBaru) > budgetGlobal) {
                    double maksimalBisaDiinput = budgetGlobal - totalAlokasiLain;
                    if (maksimalBisaDiinput < 0) maksimalBisaDiinput = 0; 
                    JOptionPane.showMessageDialog(this, 
                        "🚨 GAGAL SIMPAN ALOKASI!\n\n" +
                        "Total jatah kategori akan melebihi Budget Total Bulananmu.\n" +
                        "Sisa ruang budget yang bisa diisi untuk [" + namaKatTerpilih + "] maksimal:\n" +
                        "Rp " + String.format("%,.0f", maksimalBisaDiinput).replace(",", "."), 
                        "Peringatan Logika Keuangan", JOptionPane.WARNING_MESSAGE);
                    return; 
                }

                DatabaseHelper.updateBudgetKategori(idKatTerpilih, inputBatasBaru); 
    ...
    private void refreshHistoriTabel() {
        try {
            if (tabelHistori != null) {
                DefaultTableModel model = DatabaseHelper.getHistoriTableModel();
    ...
    private void simpanData() {
        try {
            ...
            new Pengeluaran(txtNamaBarang.getText(), nominalInput, idKat, tglInput).simpan(); 
        ...
        if (JOptionPane.showConfirmDialog(this, "Catat pembelian [" + qty + " " + namaBarang + "] ke pengeluaran budget?", "Integrasi ERP", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            try {
                ...
                if (JOptionPane.showConfirmDialog(this, popForm, "Catat Kas", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
                    ...
                    new Pengeluaran(namaBarang, nominalInput, idKat, txtTglPop.getText()).simpan();
